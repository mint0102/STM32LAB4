 ...
 * Implement an array of scheduler instances with the expectation of achieving O (1) complexity in the SCH_Update function.
 * If there is a task to be executed, the delay of that task is calculated after the previous task is executed.
 * => In function SCH_Update, we only need implement task at index = 0;
 	 - SCH_Update is called in function
 	 - HAL_TIM_PeriodElapsedCallback() every 10ms.
 * In function SCH_Add_Task, we find the satisfying locations to add the new task and recalculate time delay of all task in the scheduler.
 * With this idea, function SCH_Dispatch_Tasks() only need check RunMe of the first task in the scheduler.
 */
...
unsigned char SCH_Add_Task(void (*pFunction)(), uint32_t DELAY, uint32_t PERIOD) {
	...
	if (current_index_task == 0){
		// If the scheduler is empty, add task index = 0
